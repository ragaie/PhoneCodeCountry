...
public protocol CountryCodeDropDownDelegate {
    func codeDropDown(_ fromView: CountryCodeDropDown, didSelectItem country: Country)
}

@objc
@IBDesignable open  class CountryCodeDropDown: UIView ,UITableViewDelegate,UITableViewDataSource{
    ...
    public var delegate : CountryCodeDropDownDelegate!
    ...
    public func initSubviews() {
        ...
        let nib = UINib(nibName: "CountryCodeDropDown", bundle: bundle)
        ...
        addSubview(view)
        codeLabel.adjustsFontSizeToFitWidth = true
        tapGesture = UITapGestureRecognizer(target: self, action: #selector(CountryCodeDropDown.tapBlurButton(_:)))
    ...
    func initActionAndDelegete()  {
        actionButton.addTarget(self, action: #selector(CountryCodeDropDown.showCountryCodes(_:)), for: .touchUpInside)
    ...
    @objc func showCountryCodes(_ sender : UIButton){
        ...
        if showDrop == true {
            ...
            if containerView == nil {
                ...
                filterTable.tag = 1
                let bundle = Bundle(for: type(of: self))
                tableData.register(UINib(nibName: "CodeTableCell", bundle: bundle), forCellReuseIdentifier: "cellID")
    ...
    public func  tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if tableView.tag == 0 {
            ...
            flageIcon.image =  UIImage.init(named: countries[indexPath.row].code, in: bundle, compatibleWith: UITraitCollection?.none)
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView.tag == 0 {
            ...
            let bundle = Bundle(for: type(of: self))
            cell.flageImage.image =  UIImage.init(named: countries[indexPath.row].code, in: bundle, compatibleWith: UITraitCollection?.none)//(named: countries[indexPath.row].code, in: bundle, with: nil)
    ...
    private func getAllCountries()-> [Country]  {
        ...
        do {
            let bundle = Bundle(for: type(of: self))
            if let file = bundle.url(forResource: "countryCodes", withExtension: "json") {
